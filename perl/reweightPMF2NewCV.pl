...
|   the time series of a trajectory file using the biasing potential.                        |
|                                                                                            |
...
usage:
  1D: mapPMF2NewCV.pl nx minx maxx inputFE inputHisF xcolumn fcolumn nf minf maxf output KT
  2D: MapPMF2NewCV.pl nx minx maxx ny miny maxy inputFE inputHisF xcolumn ycolumn fcolumn nf minf maxf output KT 
end_of_usage
...
if($#ARGV==15)
{
    &FE2DTo1D($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3],$ARGV[4],$ARGV[5],$ARGV[6],$ARGV[7],$ARGV[8],$ARGV[9],$ARGV[10],$ARGV[11],$ARGV[12],$ARGV[13],$ARGV[14],$ARGV[15]);
}
elsif($#ARGV==11)
{
    &FE1DTo1D($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3],$ARGV[4],$ARGV[5],$ARGV[6],$ARGV[7],$ARGV[8],$ARGV[9],$ARGV[10],$ARGV[11]);
}
else
...
sub FE1DTo1D {
	...
	if ($tmpx<$nx && $tmpx>=0 && $tmpf<$nf && $tmpf>=0)
	{
	   ...
	   # $sumf[$tmpf] += $arrayF[$tmpx];
           # weight is required, bias potential is the negative value of PMF from metadaynamics  
           $histo[$tmpf] += exp(-$arrayF[$tmpx]/$KT);
	   $sumf[$tmpf] += $dataf[$counter]*exp(-$arrayF[$tmpx]/$KT);
    ...
    {   
	my $Prob =  $histo[$counter]/$sumHis;
	my $tmpf=$minf+$df*$counter;
	if ($histo[$counter] != 0)
	{  $sumf[$counter] /= $histo[$counter];
	   printf OUTPUT " %20.10f %20.10f %20.10f %20.10f \n", $tmpf, $Prob,-$KT*log($Prob), $sumf[$counter];
	}
    }
    close(OUTPUT);
...
## I,J GOING FROM 1 TO NX AND 1 TO NY
	...
	if ($tmpx<$nx && $tmpx>=0 && $tmpy<$ny && $tmpy>=0 && $tmpf<$nf && $tmpf>=0 )
	{
	   ...
	   # $sumf[$tmpf] += $arrayF[$x];
	   #  weight is required 
            $histo[$tmpf] += exp(-$arrayF[$x]/$KT);
	    $sumf[$tmpf] += $dataf[$I]*exp(-$arrayF[$x]/$KT);
    ...
    { 
	my $fval=$minf+$df*$i;
        my $Prob = $histo[$i]/$sumHis; 
        if ($histo[$i] != 0) 
	{
	    $sumf[$i] /= $histo[$i]; 
	    printf OUTPUT " %20.10f %20.10f %20.10f %20.10f \n", $fval, $Prob,-$KT*log($Prob),$sumf[$i];
	}	
    }
    close(OUTPUT);
