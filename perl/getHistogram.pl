...
my $head = <<end_of_head;
----------------------------------------------------------------------------------------------
|   This is a perl program to calculate the histogram or probability distribution in one     |  
|   dimension or two dimensions from time series.                                            | 
|                                                                                            |
...
usage:
  1D: getHistogram.pl nx minx maxx inputx xcolumn output
  2D: getHistogram.pl nx minx maxx inputx xcolumn ny miny maxy inputy ycolumn output 
end_of_usage
...
if($#ARGV==10)
{
    &hist2d($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3],$ARGV[4],$ARGV[5],$ARGV[6],$ARGV[7],$ARGV[8],$ARGV[9],$ARGV[10]);
}
elsif($#ARGV==5)
{
    &hist1d($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3],$ARGV[4],$ARGV[5]);
}
else
...
sub hist1d {
    ...
    my $output=$_[5];     # the file name of output of the histogram 
    my @datax;            # array to store the input data

    open(INPUT, "$inputx");
    LINEX: while(<INPUT>) #read in data
    {
	chomp;
	...
	printf OUTPUT "$tmp    $histo[$counter] \n";
    }
    close(OUTPUT);

    print " The histogram data has been written to the file, $output.\n";
    ...
    print " The histogram in one dimension has been plotted into the file, $plotname.ps.\n";
...
sub hist2d {
    ...
    my $output=$_[10];    # the file name of output of the histogram 
    ...
    open(INPUT, "$inputy");
    LINEY: while(<INPUT>) #read in data
    {
	chomp;
...
## I,J GOING FROM 1 TO NX AND 1 TO NY
    ...
    {  my $xval=$minx+$dx*($x)-$dx/2;
	   ...
	   print OUTPUT "$xval    $yval    $histo[$tmp1]\n";
       }
       print OUTPUT "\n";
    }
    close(OUTPUT);
    ...
    close(OUTPUT);
    system "gnuplot template.gnu";
    print " The histogram in two dimensions has been plotted into the file, $plotname.ps.\n";
