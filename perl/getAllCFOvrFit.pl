...
############################################################################################################
# This a perl program to fit all correlation functions of overall motion upto five exponential components  #
# by calling gnuplot.                                                                                      #
#                                                                                                          #
#  1) Co(t) = exp(-t/t1)                                                                                   # 
...
print $myhead; 
print "\n";
print " NOTE: Unlike getCFOvrFit.pl, this program fits all data files related to all vectors. \n";
...
my $outputfile_parm = "AllCFOvrFitted_parm.dat";
my $outputfile_erro = "AllCFOvrFitted_erro.dat";
my $maxIniGuess = 50; 
my $maxIniRefine = 500; 
...
for (my $ifc = 0; $ifc <= $#CFfiles; $ifc++)
{
  ...
  system "grep -A 1000000 \"0.0000\" < temp.dat > $datafile";

  #initial fitting 
  system "getCFOvrFit.pl $nexp $parmGuess $tstart $tend $datafile >& $base\_ind.log";
  ...
  while ($initialSuccess > 0 && $ninitial <= $maxIniGuess)
  {
	  ...
	  system "getCFOvrFit.pl $nexp $parmsTry $tstart $tend $datafile >& $base\_ind.log";
	  $ninitial = $ninitial + 1; 
	  if ($ninitial % 50 == 0)
	  {
